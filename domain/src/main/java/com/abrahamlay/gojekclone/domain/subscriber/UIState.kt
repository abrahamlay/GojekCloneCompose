...
package com.abrahamlay.gojekclone.domain.subscriber
...
fun <T> UiState<T>.copyWithResult(value: ResultState<T>): UiState<T> {
    return when (value) {
        is ResultState.Success -> copy(loading = false, exception = null, data = value.data)
        is ResultState.Error -> copy(loading = false, exception = value.throwable)
        else -> copy(loading = true, exception = null, data = null)
    }
}